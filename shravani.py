...
try:
	connection = mysql.connector.connect(host = 'mydatabase.c3ao3x2iwh4o.ap-south-1.rds.amazonaws.com',
                                        database = 'employee_db2',
                                        user = 'shravani',
                                        password = 'shravani16')
	mysql_empsql_insert_query = "INSERT INTO employee(empid, empname, empaddress) VALUES (%s, %s, %s)"
	rows = []
	with open('projectfile.csv', newline = '') as csvfile:
		reader = csv.DictReader(csvfile)
		for row in reader:rows.append(row.values())
